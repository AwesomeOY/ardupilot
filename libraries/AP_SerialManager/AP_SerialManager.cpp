        ...
        if (state[i].uart != nullptr) {
            switch (state[i].protocol) {
                ...
                case SerialProtocol_ESCTelemetry:
                    ...
                    state[i].uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
				/* µõ²ÕSCA150I  add by awesome */
				case SerialProtocol_SCA150I:
					state[i].baud = AP_SERIALMANAGER_SCA150I_BAUD / 1000;   // update baud param in case user looks at it
					state[i].uart->begin(map_baudrate(state[i].baud),
										 AP_SERIALMANAGER_SCA150I_BUFSIZE_RX,
										 AP_SERIALMANAGER_SCA150I_BUFSIZE_TX);
					break;
            }
        }
    }
